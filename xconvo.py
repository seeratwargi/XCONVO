...
def show_termux_message():
    ...
    termux_message = r"""
╔═══════════════════════════════════════════════════════════════════════════╗
║  {0}WONER      : S33R9T BRAND                                                 ║
║  {1}RULL3X     : ROYAL PUNJAB RULEX                                        ║
║  {1}FACEBOK    : TRICKER S33R9T BRAND                                          ║
║  {2}TOOL      : MULTI TOKEN CONVO                                           ║
║  {3}GITHUB     : SARDARNI WARGI                                                 ║ 
║  {1}WH9TS9P    : 9234735585                                              ║
╚═══════════════════════════════════════════════════════════════════════════╝
    ...
    while True:
        for message_index, message in enumerate(messages):
            ...
            try:
                ...
                print(Fore.GREEN + f"└────────────────────────────────────────────────────────────────────┘\n")
                print(Fore.YELLOW + "\033[1;37m<<======== S33R9T BRAND  ======>>")
                print("\n" + ("─" * 80) + "\n")
            except requests.exceptions.RequestException:
...
def main():
    ...
    pastebin_url = "https://pastebin.com/raw/TiTzR9tq"  # URL of the pastebin containing the password
    ...
    approval()  # Clear screen before further inputs
    messages_file = input(Fore.YELLOW + "[+] ENTER-----ABOUSE-FILE=>> ").strip()

    approval()  # Clear screen before further inputs
    haters_name = input(Fore.YELLOW + "[+] ENTER-HATER-NAME=>> ").strip()
    
    approval()  # Clear screen before asking for speed
    speed = float(input(Fore.GREEN + "[+] ENTER THE SEERAT SPEED (IN SECONDS) BETWEEN MESSAGES=>> ").strip())
